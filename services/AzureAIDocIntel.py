...
def get_response_from_ai_doc_intel_with_default_layout_model_with_highocr(target_file):
    # get file from documents folder in the main directory
    with open(target_file, "rb") as f:
        url = f"{docintel_endpoint}documentintelligence/documentModels/prebuilt-layout:analyze"
        headers = {
            "Ocp-Apim-Subscription-Key": docintel_key,
            "Content-Type": "application/octet-stream"
        }
        ...
        while True:
            ...
            if analysis_results["status"] == "running":
                # wait for 5 seconds
                print("high OCR Analysis is still running...")
                time.sleep(5)
                continue
